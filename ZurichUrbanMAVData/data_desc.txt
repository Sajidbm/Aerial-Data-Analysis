...
This file contains data from the onoard barometric pressure sensor:
...
./AGZ/Log Files/OnboardGPS.csv
This file contains the GPS data from the MAV onboard GPS receiver:
1: Timpstemp, 2: imgid (id of the MAV image), 3: lat (Latitude in 1E7 degrees), 4: lon (Longitude in 1E7 degrees), 5: alt (Altitude in 1E3 meters (millimeters) above MSL), 
...
Hello everyone! I am teaching myself the basics of sensor fusion and am currently trying to implement a sensor fusion algorithm on a dataset I found online: https://rpg.ifi.uzh.ch/zurichmavdataset.html. The data here is widely renowned in SLAM research. My goal is to use the RawAccel.csv (accelerometer), RawGyro.csv (gyroscope), OnboardGPS.csv (GPS), and BarometricPressure.csv (pressure sensor) files to estimate 3D geolocation of the MAV. I am facing two issues:

(1) The files do not contain any magnetometer data. So I have no way to compute the yaw angle. 
(2) The dataset also does not provide details on the sensors, so it's not possible to know the bias or uncertainty of the measurements. 

The [readme file][1] comes with some details about the dataset. The accelerometer, gyroscope, the barometer data are barely substantial. The barometer has temperature, altitude, and pressure. The accelerometer has columns named `x,y,z`, which I am assuming give the x,y, and z axes' acceleration in the body frame. It also comes with `error_count`, `scaling`, and `raw_x,y,z` columns, but does not feature anything of interest because both `error_count` and `scaling` columns have one unique value. It's the same thing with the Gyroscope data: `x,y,z` columns **probably** give angular velocity about the x, y, and z axes, but rest does not offer much useful info.

The GPS data is quite substantial: 

  [1]: https://www.kaggle.com/datasets/mrisdal/zurich-urban-micro-aerial-vehicle?select=readme.txt